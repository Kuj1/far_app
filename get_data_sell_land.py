...
from app.models import CardsSellLand
from app import db


# Constants
UA = UserAgent(verify_ssl=False)
URL = 'https://www.farpost.ru/primorskii-krai/realty/land/'
    ...
    try:
        ...
        for check_page in range(1, int(pagination) + 1):
            ...
            for check_link in all_cards:
                ...
                with open(f'{dir_path}/card_sell_land.html', 'w', encoding='utf-8') as doc:
                    doc.write(driver.page_source)

                with open(f'{dir_path}/card_sell_land.html', 'r') as src:
                    src = src
                ...
                contacts_raw = info.find('div', class_='new-contacts dummy-listener_new-contacts').text.replace('\t',
                                                                                                '').replace('\n', ' ')
                # Add data in DB
                card = CardsSellLand(link_obj=card_page, views_info=views_info, card_number=card_number_raw, section=section_raw, title=title_raw,
